...
def get_one_stock(stock):
	...
	if float(price)>=ma5 and diffp <= 1:
		print u"  %5d  %s%s  %6s  %5s | %5.2f %5.2f%s" % (stock, " "*(3-len(stockName))*2, stockName, price, todayDiff, ma5, diffp, "%")
...
"""
def check_amount( stockNumber, index ):
...
def get_list( requestURL, index ):
	...
	for line in webContent.splitlines():
		...
		if addIntoList1 == 1 :
			addIntoList2, addIntoList3 = check_amount( stockNumber, index )
